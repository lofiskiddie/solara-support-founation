...
do 
	...
	do 
		...
		do 
			...
			utility.drawings["Sub"] = utility:drawObject("Text", {Outline = true, Text = "foundation", Color = Color3.new(1, 1, 1), Size = 13})
			utility.drawings["Domain"] = utility:drawObject("Text", {Outline = true, Text = "hvh", Color = Color3.fromRGB(132, 108, 188), Size = 13})
			--
			...
			do 
				Messages = {
					...
					["Main"] = {
						"Owned By Foundation!!!",
						"i Resolve Your bloodline!!",
						"B4B DEMONS LOFI AND VIKO",
						"avas a groomer",
						"AZUREK and endlessK",
						"BEST HVH BOT$",
						"azure cant keep up:3",
						"COME COP",
						"Pooron!!!",
						"brokie",
						"i love foundation",
						"notice how i resolved u!!",
						"buddy u blow!!",
						"u cant tap but i can!!",
						"boohoo get outta here with ur network!!",
						"cant hit me man!!!",
					},
				}
				utx = {
		...
		do 
			...
			function HitReg:HitSound(Id, Volume, Pitch)
				...
				--[[writefile("song.mp3", game:HttpGet("https://foundation.feet.media/osiris/cmwr7ecb.mp3"))
...
local Window = Library:Window({Size = UDim2.new(0,870,0,600)}) do 
	-- // Rage
	local aiming = Window:Page({Name = "Redirection", Weapons = true}) do
		local Settings = aiming:Weapon({Icon = "rbxassetid://17373013220"})
		local Visuals = aiming:Weapon({Icon = "rbxassetid://7300480952"})
		-- 
		local mainTarget = Settings:Section({Name = "Toggle", Size = 330}) do 
			mainTarget:Toggle({Name = "Enabled", Flag = "Silent Enabled", Risk = true})
			mainTarget:Keybind({Name = "KeyBind", Flag = "Silent_Bind", Mode = "Toggle", callback = function()
		...
		local targetSettings = Settings:Section({Name = "Settings", Size = 330}) do 
			targetSettings:Toggle({Name = "Look At Target", Flag = "Look At Target"})
            ...
            targetSettings:Toggle({Name = "Tripple prediction", Flag = "Tripple"})
            targetSettings:Toggle({Name = "Customs bypass", Flag = "customs"})
            targetSettings:Toggle({Name = "Cc flag prediction", Flag = "Aim Viewer Bypass"})
            targetSettings:Toggle({Name = "Silent Cam", Flag = "Aim Viewer Bypass"})

		end 
		-- 
		local targetMethods = Settings:Section({Name = "velocity methods", Size = 330, Side = "right"}) do 
		...
		-- 
		local targetPrediction = Settings:Section({Name = "Prediction", Size = 830, Side = "Left"}) do 
		...
		-- 
		local targetAutoSelect = Settings:Section({Name = "sticky Fov", Size = 330, Side = "Right"}) do 
	...
	-- // Legit  
	local legit = Window:Page({Name = "Closet"}) do 
		local Vertical;
		local Horizontal;
		local Smoothness;
		-- 
		...
		aimAssist:Toggle({Name = "Customs Bypass", Flag = "Customs"})
		-- 
	...
	local misc = Window:Page({Name = "Misc", Weapons = true}) do 
		...
		local chatSpam = Movement:Section({Name = "Chat Spam", Size = 330, Side = "Left"}) do
			...
			chatSpam:Dropdown({Name = "Type", Flag = "Chat Spam Type", Options = {"foundation", "Scottish"}, Default = "foundation"})
	...
	local Config = Window:Page({Name = "Settings"}) do
		...
		--
		local CurrentList = {};
		if not isfolder("foundation") then 
			makefolder("foundation")
		end 
		if not isfolder("foundation/Configs") then 
			makefolder("foundation/Configs")
		end 
		local function UpdateConfigList()
			local List = {};
			for idx, file in ipairs(listfiles("foundation/Configs")) do
				local FileName = file:gsub("foundation/Configs\\", ""):gsub(".cfg", "")
		...
		Cfgs:Button({Name = "Create", Callback = function()
			...
			if ConfigName == "" or isfile("foundation/Configs/" .. ConfigName .. ".cfg") then
				return;
			end;
			writefile("foundation/Configs/" .. ConfigName .. ".cfg", Library:GetConfig());
			UpdateConfigList();
		end})
		Cfgs:Button({Name = "Save", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				writefile("foundation/Configs/" .. SelectedConfig .. ".cfg", Library:GetConfig())
			end;
		end})
		Cfgs:Button({Name = "Load", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				Library:LoadConfig(readfile("foundation/Configs/" .. SelectedConfig .. ".cfg"))
			end
		end})
		Cfgs:Button({Name = "Delete", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				delfile("foundation/Configs/" .. SelectedConfig .. ".cfg")
			end
			UpdateConfigList();
		end})
...
Library:Notification("Loaded Foundation in " .. Floor((tick() - loadingTime) * 1000)  .."ms", 3, Library.Accent, flags["Notification Position"])
Library:Notification("WARNING: chance of ban USE ON A ALT", 3, Library.Accent, 5, Color3.new(1, 0, 0), flags["Notification Position"])
delay(3, function()
	Library:Notification("Message from Lofi\n Desync NOT WORKING CORRECTLY ", 10, Library.Accent, flags["Notification Position"])
end)
